...
SKILL_SEEDS = [
    ...
    # P5 验证
    {"name": "P-migration-verification", "series": SkillSeries.P, "category": SkillCategory.p5, "description": "P5 验证主工作流：验证迁移结果正确性(编译/运行/测试/等价)回答\"改对了吗\"，给 P4 产物+源→按需读→规划本次该验哪些维度→确定性工具产铁证(编译器/测试/diff/DB/浏览器)→解读失败+提回 P4 rework 修复建议；确定性产事实、LLM 只给判断绝不产\"通过\"替代真测、No Evidence No Completed、失败只回 P4 不在 P5 改码、只产单一 p5_validation_report.json 不新增槽位/Gate——rebuild R17.5(D-108)", "status": SkillStatus.platform_runtime, "skill_source": "rebuild", "directory_path": f"{_S_DIR}/p5/P-migration-verification/"},
    ...
    # P6 交付
    {"name": "P-migration-delivery", "series": SkillSeries.P, "category": SkillCategory.p6, "description": "P6 交付主工作流：将 P4 真实产物 + P5 真实验证结论整理为可移交/可审计/可复查/可回退的最终交付包，回答\"交付了什么、能交付吗(PoC 还是 Production)\"；确定性内核产事实与门禁(清单/hash SHA-256/脱敏/许可检测/AETA 索引/P5→P6 双向门禁/final gate)、LLM 只给交付叙述+验收建议(advisory)绝不产\"通过/accepted\"替代门禁；无 P5 验证即 accepted/无许可即分发/PoC 冒充 Production/隐藏缺口/含明文密钥/无回退却称可交付=一票否决；只产单一 p6_delivery_report.json、source/ 无条件排除、下载脱敏+SHA-256——rebuild R17.5(D-108)", "status": SkillStatus.platform_runtime, "skill_source": "rebuild", "directory_path": f"{_S_DIR}/p6/P-migration-delivery/"},
...
RESOURCE_SEEDS = [
    ...
    {"resource_type": ResourceType.skill, "name": "P 系列平台 Skill（30 个）", "status": ResourceStatus.active, "risk_level": RiskLevel.L1, "source_type": SourceType.community, "source_trust_level": TrustLevel.reviewed_reference, "source_path_or_ref": f"{_S_DIR}/", "description": "24 个源自 ECC(MIT) 改写为信创迁移语境 + 6 个 rebuild 自有 P 阶段主工作流 skill（full-stack-profiler P1 建档 / P-migration-assessment P2 评估 / P-migration-planning P3 规划 / P-migration-execution P4 执行 / P-migration-verification P5 验证 / P-migration-delivery P6 交付，均 D-108），覆盖 P0-P6 + 跨阶段，均有真实 SKILL.md；原 30 候选剔除 6 个非迁移类（市场对标/营销/UI美学/harness运维/工具选型/运维监控）", "type_metadata": {"count": 30, "source_repo": "https://github.com/affaan-m/ECC", "license": "MIT", "dropped_non_migration": ["benchmark-methodology", "content-engine", "design-system", "config-gc", "agent-eval", "dashboard-builder"]}},
    ...
    # B-ACC-NO-READONLY-FILEGLOB 解除条件①：补 L1 递归**文件名**检索能力。
    # 此前 list_files 不递归、code_grep 只搜文件内容、fs_read 需已知确切路径 ⇒
    # "在整棵源码树里按名找 *.csproj/*.sln" 这个纯只读操作在 L1 无对应能力，模型被迫去调
    # L4 的 run_safe_command 跑 find（真跑 gate-22343c 由此产生）。补它**不触碰任何安全边界**，
    # 且 `run_safe_command` 的 L4 风险位**不降**（Q-ACC-2 复核已推翻降级方向）。
    {"resource_type": ResourceType.tool, "name": "find_files", "description": "在 workspace 内按文件名 glob 模式递归检索（只读；只返回路径，不读内容）", "status": ResourceStatus.active, "risk_level": RiskLevel.L1, "type_metadata": {"write_scope": "none", "binds_via": "execution_provider(R8)", "parameters": {"type": "object", "properties": {"pattern": {"type": "string", "description": "文件名 glob，如 *.csproj；多个模式用逗号分隔，如 *.sln,*.csproj"}, "path": {"type": "string", "description": "检索起点目录，相对 workspace 根，默认 source/"}, "max_results": {"type": "integer", "description": "最多返回条数（上限 300）"}}, "required": ["pattern"]}}},
    ...
    # B-ACC-NO-READONLY-FILEGLOB 解除条件②：显式声明 write_scope。
    # 它是 7 个工具里唯一能执行任意命令（白名单含 python3 ⇒ 可任意代码执行、因而可任意写）
    # 的那个，偏偏此前是唯一**没有**声明 write_scope 的，靠 tool_registry 里两处
    # `tool_name == "run_safe_command"` 工具名特判绕过。声明其真实能力为 "execute" 后，
    # 分派判定只看 write_scope（见 tool_registry._effective_write_scope）。
    # `dry_run_supported` 仍为 False —— 本仓**没有任何**代码消费该字段（实测零 consumer），
    # 把它改成 True 只会产出一个不存在的能力声称（D-049 禁止），故不改。
    {"resource_type": ResourceType.tool, "name": "run_safe_command", "description": "运行白名单内安全命令并采集 stdout/exit_code（测试白名单+审计）", "status": ResourceStatus.active, "risk_level": RiskLevel.L4, "permission_scope": "exec_with_gate", "type_metadata": {"dry_run_supported": False, "write_scope": "execute", "requires_gate": True, "binds_via": "execution_provider(R8)", "parameters": {"type": "object", "properties": {"command": {"type": "string", "description": "要执行的命令（须在白名单内），经 ExecutionProvider 隔离执行"}}, "required": ["command"]}}},
