...
async def test_p0_p1_real_business_in_graph(realp0p1):
    ...
    assert (art / "intake_report.json").exists(), "P0 produced a real intake_report"
    ...
    intake = json.loads((art / "intake_report.json").read_text(encoding="utf-8"))
    ...
    assert "__interrupt__" in s
    # real profiler artifacts on disk
    assert (art / "profiling_summary.md").exists(), "P1 produced real profiling summary"
    assert (art / "p2_input_manifest.json").exists(), "P1 produced real P2 manifest"
    json_artifacts = list(art.glob("*.json"))
