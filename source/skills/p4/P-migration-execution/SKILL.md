...
- **给路径+按需读取，不硬编码输入**：你会拿到**源根路径**（`source/`）、**本节点绑定的源定位**（node input_refs 中的 `source/…` 路径或源根引用）、**产物输出路径**（`output_code/<node>/…`、`patches/…`）。用读取工具（`list_files`/`code_grep`/`fs_read`）**自己按需探查、定位、读取真实源**，再迁移。不要等着别人把源内容预先塞进提示词。
...
- **产物输出目标**：`output_code/<node_id>/…`（新代码唯一落区）、`patches/…`（补丁草案）。经 WorkspaceMediator 单一写闸（源只读、越界写被拒记 Audit）。
...
4. **写产出**：把迁移结果写入 `output_code/<node_id>/…`（真实扩展名 `.cs/.sql/.csproj/...`，非按标题命名的 `.txt`）；必要处写 `patches/…` 补丁草案。写盘经 Mediator；写 `source/` 会被拒（负路径，正确行为）。
...
## 输出 / 产物（固定，不发明）
- `output_code/<node_id>/…`：真实迁移代码（真实扩展名）。
