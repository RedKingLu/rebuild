...
| **阶段收口（验收 accepted）** | **依次执行收口清单：① 登记文档状态（阶段计划/总览/相关报告头部）② 在 `交接/已完成/` 下建 `<本轮>/` 子目录并移入 `交接/当前/` 本阶段材料 ③ 归档 `产物/草稿/` 本阶段材料 → `产物/已完成/` ④ 按用户指令 git 提交 ⑤ 确认 `交接/当前/`、`产物/草稿/` 已清空待下一阶段。任一项缺失即收口未完成。** |
