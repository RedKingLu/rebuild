...
> 最后更新：2026-06-25（R5-1：§2.4 更新时机规则追加进度追踪更新/阻塞登记/能力变更；§7 完成自检追加第12条追踪一致性检查；R5-4：新增 §4.4 前端施工专项规范——必读 06-UX 章节+中文优先+i18n 评估结论）
...
| **前端施工** | **§4.4（本 Skill）** | **必读 06-UX 相关章节并遵循；中文优先；能力标记/脱敏/mock 区分** |
...
> **归档落位约定**：按 **轮次子目录** 归档（`交接/已完成/R0/ R1/ R2/ R3/ R4/ V26.1.1/ …`），与 AGENTS.md §5.2 一致。往期 R0-R3 与 V26.1.1 平铺材料已于 2026-06-25 retroactively 归整到对应轮次目录；大批量子专项可在轮次目录下再建子目录（如 `R2/R2-独立验收/`）。新阶段收口时在 `交接/已完成/` 下建 `<本轮>/` 子目录并将本阶段材料整体移入。
...
> **归档触发**：与交接材料同步——**阶段完成 / 收口（验收 accepted）时**，必须把本阶段已处理完毕的 `产物/草稿/` 材料归档至 `产物/已完成/`（或已晋升至 `文档/`），使 `产物/草稿/` 只保留未完成的当前草稿。此前 §2.4 触发表只列了交接归档、未列产物归档，易被遗漏——见 §2.4 已补。
...
| 阶段完成 | 归档交接材料至交接/已完成/<轮次>/ **且** 归档产物草稿至产物/已完成/ |
| **阶段收口（验收 accepted）** | **依次执行收口清单：① 登记文档状态（阶段计划/总览/相关报告头部）② 在 `交接/已完成/` 下建 `<本轮>/` 子目录并移入 `交接/当前/` 本阶段材料 ③ 归档 `产物/草稿/` 本阶段材料 → `产物/已完成/` ④ 按用户指令 git 提交 ⑤ 确认 `交接/当前/`、`产物/草稿/` 已清空待下一阶段 ⑥ 执行追踪一致性检查（AGENTS.md §12.3；清单见 证据/进度追踪/06-状态口径与更新规则.md §3）并更新 证据/进度追踪/ 全部追踪文件。任一项缺失即收口未完成。** |
| 阶段切换 | 创建新交接提示词，写明新阶段的输入/输出/禁止/验收 |
| 决策变更 | 先按变更控制规范提出待审核修改建议；经用户确认后更新 01-决策记录.md 和变更日志 |
| 文档 accepted | 更新文档地图 |
| 发现待确认项 | 登记到总报告 + 总表 + 证据/进度追踪/03-待确认项.md |
| 本 Skill 发现不足 | 按 §8 自维护规则处理 |
| **发现阻塞项** | **登记到 证据/进度追踪/02-阻塞项.md（不得仅写在聊天或临时报告中）** |
| **能力新增/变更/废弃** | **更新 证据/进度追踪/01-功能表.md** |
...
- 跳过总报告/总表更新直接标记完成
- 归档后删除原始交接材料（应保留索引）
- **阶段收口时只做文档状态登记 / git 提交，却漏掉交接区与产物区的归档流转（§2.4 收口清单第②③项）**
...
**任何涉及前端的施工（新建/修改页面、组件、布局、文案、可视化），开工前必须读取并遵循 `文档/06-UX与前端/` 的相关章节**（详述源在该目录，本 Skill 只定义"必须读+必须遵循"，不复制规范内容）：

| 何时 | 必读章节 |
|------|---------|
| 任何前端施工 | `00-UX与前端总览.md`、`07-视觉系统与前端文案.md`（视觉系统 + 文案规范） |
| 信息架构 / 路由 / 项目区 | `01-信息架构与项目创建.md` |
| Workspace / IDE 布局 | `03-ProjectWorkspace布局规范.md` |
| 模型 / 资源 / 能力可视化 | `06-模型资源可视化与真实能力标记.md`（14 能力标记 / mock·static_demo·not_connected 区分 / 脱敏） |
| Evidence/Trace/Audit/Gate 交互 | `05-Evidence-Trace-Audit与Gate交互.md` |
| 集成页 / 聚合页 | `08-集成页设计规范.md`、`09-聚合页设计规范.md` |

**硬要求**：

```text
1. 前端用户可见文案一律「中文优先」：标题、按钮、标签、提示、空态、错误信息均用中文。
   技术标识符（provider_id / model_name / API 格式名 / 字段名 / 代码）保留原文。
2. 真实能力标记必须按 06 §2/§18 的 14 种状态（颜色+中文文字双通道），状态来自后端，不臆测。
3. mock / static_demo / not_connected 必须显式区分与标记（D-049）；已真实接入的页面不得再挂全局 mock 横幅。
4. 凭据/Key/Token 全程脱敏，前端不回显、不请求明文回传展示（AGENTS.md §12）。
5. 施工后必须做最小前后端联调（D-074）：页面真实发起 API 且渲染真实响应，并留浏览器级证据。
6. **图标统一用线性图标组件**（`frontend/src/components/ui/Icon.tsx`，VSCode Codicons 风格线性 SVG，`currentColor` 描边）：**禁止用 emoji/彩色字符（如 🔍🔑🗑✦⏳）充当功能图标**；缺图标时在 Icon 组件内新增同风格线性图标，不要内联随意图形。保证全站图标风格、线宽、字号一致。
```

**i18n（国际化）评估结论（R5-4，user_confirmed）**：当前**不引入** i18n 框架（react-i18next 等）。理由：目标用户为信创中文场景、暂无第二语言需求、R5 期界面面积小，过早抽象会增加维护成本。改以「中文优先」作为约定（见上）。**触发再评估的条件**：出现明确的第二语言需求（如英文导出/海外交付）时，再按 C2 变更引入 i18n 并抽取文案资源。登记为待确认项 Q-008（`证据/进度追踪/03-待确认项.md`）。
...
11. 是否可以交给用户审核？
12. 若本任务属于阶段收口：是否已执行追踪一致性检查并更新 证据/进度追踪/ 全部追踪文件？
```
