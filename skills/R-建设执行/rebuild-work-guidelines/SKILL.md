...
> **归档落位约定**：当前磁盘实践为 **平铺** 于 `交接/已完成/`（R0-R3 均平铺；大批量子专项可建子目录如 `R2-独立验收/`）。AGENTS.md §5.2 标注为 `交接/已完成/<轮次>/`，与磁盘平铺实践存在差异 —— 属硬规则口径问题，**待用户裁决**是否统一为平铺、或自 R5 起改用 `<轮次>/` 子目录；统一前以平铺为准、与既有 R0-R3 保持一致。
...
| 阶段完成 | 归档交接材料至交接/已完成/ **且** 归档产物草稿至产物/已完成/ |
| **阶段收口（验收 accepted）** | **依次执行收口清单：① 登记文档状态（阶段计划/总览/相关报告头部）② 归档 `交接/当前/` 本阶段材料 → `交接/已完成/` ③ 归档 `产物/草稿/` 本阶段材料 → `产物/已完成/` ④ 按用户指令 git 提交 ⑤ 确认 `交接/当前/`、`产物/草稿/` 已清空待下一阶段。任一项缺失即收口未完成。** |
