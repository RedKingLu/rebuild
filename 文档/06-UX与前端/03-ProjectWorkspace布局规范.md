...
> 修订说明：R2 校准（Q4=A）：UX 设计内容旧 F 命名统一为 P0-P6；R2 去重(Q5=B)：样板块补引用 AGENTS§18/01-决策记录，收敛通用复述，安全红线就地保留；R3-4 校准(2026-06-25)：§30.2.1 记录 R3-4 实现结果（顶部状态栏补齐 P0-P6 节点指示 + 执行模式三按钮 + 网关/模型状态；活动条三区折叠按钮统一）。
...
#### 30.2.1 R3-4 校准结果（2026-06-24，体验壳）

```text
1. 七区布局：可用，全屏 IDE 渲染正常（Playwright 实证 0 console error）
2. 顶部状态栏：已补齐——返回平台 + 项目名 + P0-P6 节点指示条（连接线 + 当前阶段高亮）
   + 阶段 chip + Gate 状态 chip + 网关/模型状态 chip（网关标记为 Mock，未接 ModelGateway）
   + 执行模式三按钮切换（自动 / 计划确认 / 手动，D-025）。不显示 mission_id / 密钥（D-070/§4.197）
3. 活动图标条：6 活动项 + 左/下/右三区折叠按钮（图标统一 panelLeft/panelBottom/panelRight，
   active 高亮）；再次点击当前活动图标折叠左侧面板（VSCode 行为，§3.3）
4. 中央类型单例 Tab、Agent 对话常驻、底部 Dock、右侧 Gate/Evidence/Trace/Audit/Ctx/Res/Mdl 检视：均可用
5. mock：GlobalMockBanner + 各区 Mock/占位/未接服务 标识齐全（D-049）
6. 执行模式当前为前端状态（execMode），真实策略联调留待 R4/R8
说明：项目区（/projects/:id）已收敛为仅创建与展示，Stage/Run/AET/Gate/设置入口统一在 Workspace（见 01-信息架构 §IA 校准）
```
