...
> 平台名称：rebuild
> 当前版本：V26.3
> 来源草稿：`产物/已完成/R1/00-API与集成契约总览.md`（v0.1）
> 最后更新时间：2026-06-24
...
## 附：关于本目录中的「R2 待校准」标记（2026-09-16，R23 登记）

**事实**：本目录多份契约文档在字段说明中写有「必填或可选待 R2 校准」一类标记（`02` / `03` / `05` 等，全库同类标记实测 108 处、跨 18 份文档）。**R2 已于 2026-06-24 收口**，这些标记从未在 R2 被逐项关掉。

**处置（R23 不越权改写）**：字段的**必填/可选、类型与取值口径，实际以后端已实现的 Pydantic schema 与路由为准**（`backend/app/schemas/`、`backend/app/routes_*.py`）——契约文档是"建议契约"，实现已经先行落地并经 R4~R22 多轮验收。R23 是文档核对阶段，**不擅自把"待校准"改写成某个具体结论**（那需要逐字段对照代码并可能改变契约语义，属实质结论变更）。

⇒ 该项已登记为需要一次「文档契约 ↔ 已实现 schema」逐字段对账的独立工作，**归口 R24**（代码与架构核对阶段，届时可同时看到两侧）。在对账完成前：

```text
读者口径：契约文档给的是设计意图与字段语义；
          必填性/类型的当前真值以 backend/app/schemas/ 为准；
          两者冲突时以实现为真，并把冲突登记为缺陷（而非默认文档为真）。
```
